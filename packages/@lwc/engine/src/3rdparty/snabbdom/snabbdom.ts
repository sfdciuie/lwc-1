...
interface NodeOperations<HostNode> {
...
export function createPatchFunction(nodeOperations: NodeOperations<Node>) {
    type HostVNode = VNode<Node>;
    type HostVNodes = VNodes<Node>;

    const { nextSibling } = nodeOperations;
    ...
    function updateStaticChildren(parentElm: Node, oldCh: HostVNodes, newCh: HostVNodes) {
        const { length } = newCh;
        if (oldCh.length === 0) {
    ...
    function updateDynamicChildren(parentElm: Node, oldCh: HostVNodes, newCh: HostVNodes) {
        let oldStartIdx = 0;
        let newStartIdx = 0;
        ...
        let oldKeyToIdx: any;
        let idxInOld: number;
        let elmToMove: HostVNode | null | undefined;
        let before: any;
